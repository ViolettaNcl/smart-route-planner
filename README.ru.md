...
<p>
  ...
  <img src="https://img.shields.io/badge/tests-132_passing-success?style=flat-square" alt="132 теста">
...
Пользователь вводит список городов — сервис их геокодирует, **сам определяет
эффективный порядок объезда** (а не едет в том порядке, в котором города были
введены), **предсказывает подходящий транспорт нейросетью, обученной с
нуля**, показывает маршрут на **интерактивной карте** и формирует **AI-совет
по поездке** (привалы, ночёвки, погода) через LLM.
...
- **Инженерная зрелость помимо ML** — 132 автотеста (unit + HTTP-интеграция),
  свой rate limiter (token bucket, а не наивное фиксированное окно), CI-матрица
...
- **Интерактивная карта** на Leaflet.js — пронумерованные маркеры, реальная
  геометрия пути, без API-ключа для карты.
...
- **Автоподсказки городов** — живой поиск через Nominatim, навигация
  клавиатурой.
...
- **Светлая / тёмная тема** — сохраняется между визитами; тайлы карты и
...
- **132 автотеста** — 106 unit-тестов (поддельные зависимости, без I/O) и 26
  HTTP-интеграционных (см. [Тесты](#тесты)).
...
explain/assistant «живьём») — 132 теста в сумме. Гоняются автоматически в CI
...
- История маршрутов не сохраняется (кроме шеринга через ссылку) — задел на
...
- Автоподсказки городов и сам расчёт маршрута требуют доступа в интернет
  (Nominatim/OSRM) — без сети форма сообщит об этом, а не упадёт молча.
