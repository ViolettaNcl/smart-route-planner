...
5. `app.js` обновляет карточки результата (включая время, стоимость, CO2) и
   перерисовывает векторную карту MapLibre GL JS — линия маршрута рисуется по настоящей
   ...
   городами. Стиль OpenFreeMap (тёмный/светлый) и цвета графика границы
   решений синхронизируются с темой интерфейса через `ui.js`; отдельный
   контрол переключает плоский 2D-вид и перспективный 3D-вид со зданиями.
