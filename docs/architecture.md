...
Переход от одного процедурного файла к слоям с чёткой ответственностью.
Ничего не выполняется на верхнем уровне файлов класса — каждый класс просто
предоставляет метод, поэтому всё, кроме HTTP-обвязки, покрыто тестами без
...
public/
  ...
  assets/
    css/route.css
    js/
      app.js                    # Оркестрация фронтенда: fetch к API, состояние формы, обработчики
      ui.js                      # Рендер результатов, карточек, карты, виджетов (POI/погода/план по дням)
    ...
    icons/                      # Иконки PWA (192/512/512-maskable)
src/
...
5. `ui.js` обновляет карточки результата (включая время, стоимость, CO2) и
   перерисовывает карту Leaflet — линия маршрута строится по настоящей
   геометрии дороги, если она есть, иначе по прямым между городами.
...
| `public/assets/js/app.js` | Оркестрация фронтенда: fetch к API, состояние формы, обработчики событий |
| `public/assets/js/ui.js` | Рендер результатов, карточек, карты и виджетов фич |
...
- **OpenStreetMap Tile Server** — тайлы карты для Leaflet (`{s}.tile.openstreetmap.org`).
