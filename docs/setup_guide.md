...
веса модели после "живого" дообучения) переживают `docker compose restart`
и пересборку образа благодаря volume в `docker-compose.yml`.
...
Должно вывести `Пройдено: 132, провалено: 0` (число тестов может увеличиться в
будущих версиях).
...
**"Файл весов модели не найден"** — вы не запустили `php bin/train_model.php`
перед первым использованием (см. шаги выше).
